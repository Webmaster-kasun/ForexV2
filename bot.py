...
# R:R TARGET = 1:1.5 on all pairs (TP always bigger than SL)
# Profit/Loss per trade at 0.10 lots:
# AUD/USD: SL=$15  TP=$20  R:R=1:1.3
# EUR/GBP: SL=$12  TP=$18  R:R=1:1.5
# USD/CHF: SL=$15  TP=$20  R:R=1:1.3
# XAG/USD: SL=$10  TP=$15  R:R=1:1.5  (100oz lot, pip=0.01)
...
ASSETS = {
    "AUD_USD": {
        ...
        "stop_pips":   15,    # Risk  $15 per trade
        "tp_pips":     20,    # Profit $20 per trade → R:R 1:1.3
        "pip":         0.0001,
        "precision":   5,
        "min_atr":     0.0004,
        "max_hours":   4,
    ...
    "EUR_GBP": {
        ...
        "stop_pips":   12,    # Risk  $12 per trade
        "tp_pips":     18,    # Profit $18 per trade → R:R 1:1.5
        "pip":         0.0001,
        "precision":   5,
        "min_atr":     0.0003,
        "max_hours":   4,
    ...
    "XAG_USD": {
        ...
        "stop_pips":   100,   # 100 pips × 0.01pip × 100oz = $10 risk
        "tp_pips":     150,   # 150 pips × 0.01pip × 100oz = $15 profit → R:R 1:1.5
        "pip":         0.01,  # Silver pip = $0.01 per oz
        "precision":   2,
        "lot_size":    100,   # 100 oz = standard silver lot
        "min_atr":     0.10,
        "max_hours":   4,
        "session_hours": [(14, 23), (0, 1)],
    },
        ...
        if position:
            ...
            if name in open_times:
                try:
                    ...
                    log.info(name + " open for " + str(round(hours_open, 1)) + "h / max " + str(max_hours) + "h")

                    if hours_open >= max_hours:
                        # Force close — trade ran too long!
                        close_result = trader.close_position(name)
                        if close_result["success"]:
                            log.info(name + " FORCE CLOSED after " + str(round(hours_open, 1)) + "h. PnL=$" + str(round(pnl, 2)))
                            # Track win/loss
                            if pnl > 0:
                            ...
                            with open(trade_log, "w") as f:
                                json.dump(today, f, indent=2)
                            alert.send(
                                "⏰ DEMO 2 TIME LIMIT CLOSE
"
                                + config["emoji"] + " " + name + "
"
                                "Reason: " + str(round(hours_open, 1)) + "h > " + str(max_hours) + "h max
"
                                "Direction: " + direction + "
"
...
"
                            ...
                            scan_results.append(config["emoji"] + " " + name + ": TIME LIMIT closed " + emoji + " $" + str(round(pnl, 2)))
                            continue
                except Exception as e:
                    log.warning("Duration check error: " + str(e))

            scan_results.append(config["emoji"] + " " + name + ": " + direction + " open " + emoji + " $" + str(round(pnl, 2)) + " (" + str(round(hours_open if name in open_times else 0, 1)) + "h)")
            continue
